...
console.log('🔧 Ajout complet du système de tri...');

// 1. D'abord, ajouter l'état sortBy (s'il n'existe pas déjà)
...
// 2. Ajouter la fonction sortCards AVANT filteredCards
if (!content.includes('const sortCards =')) {
  const oldUnique = `  // Convertir l'objet en tableau
  const uniqueCards = Object.values(groupedCards)

  const filteredCards = uniqueCards.filter(card => {`;

  const newUnique = `  // Convertir l'objet en tableau
  const uniqueCards = Object.values(groupedCards)
  ...
  content = content.replace(oldUnique, newUnique);
  console.log('   ✓ Fonction sortCards ajoutée');
}

// 3. Ajouter sortedCards après filteredCards (si pas déjà présent)
if (!content.includes('const sortedCards =')) {
  const oldReturn = `    return matchesSearch && matchesRarity && matchesCondition && matchesType
  })

  return (`;

  const newReturn = `    return matchesSearch && matchesRarity && matchesCondition && matchesType
  })
...
// 4. Remplacer filteredCards par sortedCards dans le rendu
...
console.log('   ✓ filteredCards remplacé par sortedCards dans le rendu');

// 5. Ajouter le sélecteur de tri dans l'interface
if (!content.includes('Trier par :')) {
  const oldSort = `      {/* Sort Option */}
...
console.log('✅ Système de tri complet ajouté avec succès !');
console.log('📚 Par défaut : Bloc → Extension → Numéro (récent → ancien)');
